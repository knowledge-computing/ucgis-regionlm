############################
#
# Code base from SpaBERT repo: https://github.com/zekun-li/spabert
#
############################
...
class PseudoSentenceLoader(SpatialDataset):
    def __init__(self, data_file_path,  tokenizer=None, max_token_len = 512, distance_norm_factor = 0.0001, spatial_dist_fill=10, 
        with_type = True, sep_between_neighbors = False, label_encoder = None, mode = None, num_neighbor_limit = None, random_remove_neighbor = 0.):
        ...
        super(PseudoSentenceLoader, self).__init__(self.tokenizer , max_token_len , distance_norm_factor, sep_between_neighbors )
    ...
    def load_data(self, index):
        ...
        pivot_id = line_data_dict[const.psuedo_sentence_info][const.pivot_id]
        pivot_context = line_data_dict[const.psuedo_sentence_info][const.pivot_context]
        pivot_pos = line_data_dict[const.psuedo_sentence_info][const.pivot_geometry][const.pivot_coordinates]
        neighbor_info = line_data_dict[const.neighbor_info]
        neighbor_context_list = neighbor_info[const.neighbor_context_list]
        neighbor_geometry_list = neighbor_info[const.neighbor_geometry_list]
        train_data = self.parse_spatial_context(pivot_id, pivot_context, pivot_pos, neighbor_context_list, neighbor_geometry_list, spatial_dist_fill )
        return train_data
    
    def parse_spatial_context(self, pivot_id, pivot_context, pivot_pos, neighbor_context_list, neighbor_geometry_list, spatial_dist_fill,  pivot_dist_fill = 0):
        ...
        # process pivot
        pivot_context_tokens = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(pivot_context))
        pivot_token_len = len(pivot_context_tokens)
        ...
        # prepare entity mask
        entity_mask_arr = []
        rand_entity = np.random.uniform(size = len(neighbor_context_list) + 1) # random number for masking entities including neighbors and pivot
        ...
        for neighbor_context, neighbor_geometry, rnd in zip(neighbor_context_list, neighbor_geometry_list, rand_entity[1:]):

            if not neighbor_context[0].isalpha():
                # only consider neighbors starting with letters
                continue 

            neighbor_token = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(neighbor_context))
            ...
            if const.neighbor_coordinates in neighbor_geometry: # to handle different json dict structures
                neighbor_lng_list.extend([(neighbor_geometry[const.neighbor_coordinates][0]- pivot_lng)/self.distance_norm_factor] * neighbor_token_len)
                neighbor_lat_list.extend([(neighbor_geometry[const.neighbor_coordinates][1]- pivot_lat)/self.distance_norm_factor] * neighbor_token_len)
        ...
        pseudo_sentence = pivot_context_tokens + neighbor_token_list 
        ...
        train_data['pivot_name'] = pivot_context
        train_data['pivot_pos'] = pivot_pos
        train_data['pivot_token_len'] = pivot_token_len
        train_data['masked_input'] = masked_input
        train_data['sent_position_ids'] = torch.arange(len(pseudo_sentence), dtype=torch.long) # torch.tensor(np.arange(0, len(pseudo_sentence)))
