...
class ContinuousSpatialPositionalEmbedding(nn.Module):
    def __init__(self, hidden_size):
        ...
        inv_freq = 1 / \
            (10000 ** (torch.arange(0.0, self.emb_dim) / self.emb_dim))  # (emb_dim)
    ...
    def forward(self, x):
        ...
        # outer-product, out_shape: (bsize * seq_len, emb_dim)
        flat_sinusoid_inp = torch.ger(flat_x, self.inv_freq)

        # (bsize * seq_len, emb_dim) -> (bsize, seq_len, emb_dim)
        sinusoid_inp = flat_sinusoid_inp.reshape(bsz, seq_len, self.emb_dim)

        # (bsize, seq_len, 2*emb_dim)
        ret_pos_emb = torch.cat(
...
class SpatialEmbedding(nn.Module):
    ...
    def __init__(self, config):
        ...
        # a trick to simplify the weight loading from Bert
        self.sent_position_embedding = self.position_embeddings

        self.sent_position_embedding_type = getattr(
            config, "position_embedding_type", "absolute")

        self.spatial_position_embedding = ContinuousSpatialPositionalEmbedding(
            hidden_size=config.hidden_size)
        self.spatial_position_embedding_type = getattr(
            config, "position_embedding_type", "absolute")
    ...
    def forward(
        ...
        if self.use_spatial_distance_embedding:
            if len(position_list_x) != 0 and len(position_list_y) != 0:
                if self.spatial_position_embedding_type == "absolute":
                    pos_emb_x = self.spatial_position_embedding(
                        position_list_x)
                    pos_emb_y = self.spatial_position_embedding(
                        position_list_y)
                    ...
                    raise NotImplementedError(
                        "Invalid spatial position embedding type")
        ...
        if self.sent_position_embedding_type == "absolute":
            ...
            raise NotImplementedError(
                "Invalid sentence position embedding type")
            # TODO: if relative, need to look at BertSelfAttention module as well
...
class PivotEntityPooler(nn.Module):
    ...
    def forward(self, hidden_states, pivot_len_list):
        ...
        for i in torch.arange(0, bsize):
            ...
            pivot_token_tensor = torch.mean(
                torch.unsqueeze(pivot_token_full, 0), dim=1)
...
class SpatialBertOnlyTypingHead(nn.Module):
    def __init__(self, config):
        ...
        self.seq_relationship = nn.Linear(
            config.hidden_size, config.num_semantic_types)
...
class SpatialBertForSemanticTyping(SpatialBertPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            typing_loss = loss_fct(
                type_prediction_score.view(-1, self.num_semantic_types), labels.view(-1))
